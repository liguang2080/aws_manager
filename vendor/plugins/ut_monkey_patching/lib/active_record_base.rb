...
class << ActiveRecord::Base
  ...
  # 支持Article.each(:all, :conditions => ['title like ?', 'title']) {}的语法
...
class ActiveRecord::Base  
  ...
  #与linux 上的touch含义差不多.在这里主要是对时间字段的更改,就是改成最新时间
  def touch(column = "updated_at")
    self.class.update_all ["#{column} = ?",Time.now.strftime("%Y-%m-%d %H:%M:%S") ], ['id = ?', self.id] 
    self.reload
  end
end
